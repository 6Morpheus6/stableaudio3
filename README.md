# Stable Audio 3 Small Pinokio Launcher

This launcher installs and runs [Stable Audio 3](https://github.com/Stability-AI/stable-audio-3), focused on public mirrored copies of the native low-memory models:
...
The upstream `medium` model is not the default launcher path because it requires NVIDIA CUDA and Flash Attention 2. Upstream reports roughly 5.1-6.5 GB peak VRAM for `medium`, while the small models need no GPU and use about 1.7-2.4 GB peak VRAM when run on H200-class GPU. On CPU, use at least 16 GB system RAM for a comfortable experience.
